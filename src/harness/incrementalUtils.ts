...
function populateResolutionCache(
    ...
    resolutionHostCacheHost.getRootDirInfoForResolution = (_redirectedReference, resolution) => actual.sharedCache.watchedResolutionInfoMap.get(resolution)!.rootDirInfo!;
...
export function verifyResolutionCache(
    ...
    forEachOtherResolutionCachesInSharedCache(cache => {
        const expected = populateResolutionCache(cache, sharedCache);
    ...
    resolutionToRefs.forEach((info, resolution) => {
        const actualFiles = actual.filesReferencingResolution.get(resolution);
        ts.Debug.assert(
        ...
        const actualWatchedResolutionInfo = actual.sharedCache.watchedResolutionInfoMap.get(resolution)!;
        ts.Debug.assert(
        ...
        const expecedFiles = expected.filesReferencingResolution.get(resolution);
        verifySet(expecedFiles, actualFiles, `Resolution files`);
        verifyWatchedResolutionInfo(sharedCache.watchedResolutionInfoMap.get(resolution), actualWatchedResolutionInfo, "watchedResolutionInfo of Resolution");
    });
    verifyMap(sharedCache.watchedResolutionInfoMap, actual.sharedCache.watchedResolutionInfoMap, verifyWatchedResolutionInfo, "watchedResolutionInfoMap");
    ...
    function verifyResolutionSet(
        ...
        verifySet(expected, actual, `${projectName}:: ${caption}:: Resolutions`);
    }
    ...
    function verfiyResolution(
        ...
        ts.Debug.assert(
            expectedResolution ?
                ...
                sharedCache.watchedResolutionInfoMap.has(actualResolution!),
