...
export function createSharedResolutionCache(sharedCacheHost: SharedResolutionCacheHost): SharedResolutionCache {
    ...
    function clearForCache(cache: ResolutionCache) {
        ...
        watchedResolutionInfoMap.forEach((watchedResolutionInfo, resolution) => {
            if (watchedResolutionInfo.caches.has(cache)) {
                releaseResolution(resolution, /*moduleOrTypeRefCache*/ undefined);
                gcCaches = true;
            }
        });
    ...
    function isInvalidatedResolution(resolution: ResolutionWithFailedLookupLocations | undefined) {
        return !resolution ||
            ...
            !currentCache?.getValidResolution(resolution);
    }
    ...
    function invalidateResolutions(
        ...
        resolutions.forEach(resolution => {
            ...
            watchedResolutionInfo.caches.forEach(cache => cache.invalidateResolution(resolution));
        });
        return invalidated;
    }
...
export function enableSharingModuleOrTypeReferenceResolutionCache<T extends ModuleOrTypeReferenceResolutionCache<any>>(
    ...
    moduleOrTypeRefCache.setPerDirectoryAndNonRelativeNameCacheResult = (name, mode, directoryName, redirectedReference, result) => {
        setPerDirectoryAndNonRelativeNameCacheResult.call(moduleOrTypeRefCache, name, mode, directoryName, redirectedReference, result);
        withSharingModuleOrTypeReferenceResolutionCache(
            moduleOrTypeRefCache,
            sharedCache => {
                if (!sharedCache.getFromDirectoryCache(name, mode, directoryName, redirectedReference)) {
                    sharedCache.setPerDirectoryAndNonRelativeNameCacheResult(name, mode, directoryName, redirectedReference, result);
                }
            },
        );
    };
